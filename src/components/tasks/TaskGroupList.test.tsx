...
function CollapsibleGroups({ tasks, progress, searching = false, onAdvance }: GroupsProps) {
...
describe("Smoke C2: the sidebar groups real tasks correctly", () => {
  const tasks = [
  ...
  function groups(over: Partial<GroupsProps> = {}) {
    return (
      <TaskFilterProvider>
        <CollapsibleGroups tasks={tasks} progress={progress} {...over} />
      </TaskFilterProvider>
    );
  }
  ...
  // The priority and label filters moved into the sidebar header, so the list
  ...
  it("reveals a collapsed status that holds matches, and leaves it collapsed otherwise", () => {
    ...
    rerender(groups({ searching: true }));
    expect(within(column("Done")).getByText("Title task-3")).toBeTruthy();
    expect(toggle("Done").getAttribute("aria-expanded")).toBe("true");

    rerender(groups({ tasks: tasks.filter((t) => t.status !== "Done"), searching: true }));
    expect(toggle("Done").getAttribute("aria-expanded")).toBe("false");

    rerender(groups({ searching: false }));
    expect(within(column("Done")).queryByText("Title task-3")).toBeNull();
  });

  it("accepts a drop onto a collapsed status and keeps it collapsed", () => {
    const onAdvance = vi.fn();
    render(groups({ onAdvance }));
    fireEvent.click(toggle("Done"));
